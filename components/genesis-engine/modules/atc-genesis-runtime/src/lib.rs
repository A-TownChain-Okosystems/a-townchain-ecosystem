...
use atc_genesis_network::{NetworkEntity, ReplicatedState, ReplicationHeader, ReplicationTransport, Replicator, Tick};
...
impl<R, A, N: ReplicationTransport> GenesisRuntime<R, A, N> {
    ...
    pub fn queue_movement(&mut self, entity: EntityId, config: MovementConfig) -> atc_genesis_gameplay::GameplayCommand { self.gameplay.sample_movement(&self.input, entity, config) }
    ...
    pub fn play_animation(&mut self, entity: EntityId, clip: AnimationClipId, looping: bool, speed: f32) -> bool {
        ...
        self.animations.insert(entity, AnimationBinding {
            player: AnimationPlayer { clip: Some(clip), time_seconds: 0.0, speed, looping },
            clip,
    ...
    pub fn receive_secure_network_packet_for_entity(&mut self, guard: &mut PacketGuard, session: SessionId, expected: NetworkEntity, bytes: &[u8]) -> Result<(), SecureNetworkReceiveError> {
        let validated = guard.validate_for_entity(session, expected, bytes).map_err(|reason| match reason {
            PacketRejectReason::EntityMismatch => SecureNetworkReceiveError::EntityMismatch {
                expected,
                received: ReplicatedState::decode(&bytes[ReplicationHeader::BYTES..]).map(|s| s.entity).unwrap_or(expected),
            },
...
pub enum SecureNetworkReceiveError {
    ...
    EntityMismatch { expected: NetworkEntity, received: NetworkEntity },
...
fn quantize_mm(v: [f32; 3]) -> Result<[i32; 3], String> {
    if !v.iter().all(|value| value.is_finite()) { return Err("entity position contains a non-finite value".into()); }
    Ok(v.map(|x| (x.clamp(i32::MIN as f32 / 1000.0, i32::MAX as f32 / 1000.0) * 1000.0).round() as i32))
}

fn dequantize_mm(v: [i32; 3]) -> [f32; 3] { v.map(|x| x as f32 * 0.001) }

fn quantize_rotation(v: [f32; 4]) -> Result<[i32; 3], String> {
    if !v.iter().all(|value| value.is_finite()) { return Err("entity rotation contains a non-finite value".into()); }
    let norm = v.iter().map(|value| value * value).sum::<f32>();
    if !(norm.is_finite() && norm > 0.0) { return Err("entity rotation is not a valid quaternion".into()); }
    let normalized = v.map(|value| value / norm.sqrt());
    Ok([normalized[0], normalized[1], normalized[2]].map(|x| (x.clamp(-1.0, 1.0) * 1_000_000.0).round() as i32))
}

fn dequantize_rotation(v: [i32; 3]) -> [f32; 4] {
    let mut q = [v[0] as f32 / 1_000_000.0, v[1] as f32 / 1_000_000.0, v[2] as f32 / 1_000_000.0, 0.0];
...
mod tests {
    ...
    fn runtime() -> GenesisRuntime<NullRenderer> {
        GenesisRuntime::new(NullRenderer::default(), PhysicsConfig { gravity: [0.0; 3], ..Default::default() })
    }
    ...
    fn network_state_is_applied_to_existing_entity() {
        ...
        let s = ReplicatedState { entity: NetworkEntity(e.0), tick: Tick(1), position_mm: [1500, -2000, 3500], rotation_xyz_microunits: [0, 0, 0] };
    ...
    fn invalid_network_rotation_is_rejected_without_mutation() {
        ...
        let s = ReplicatedState { entity: NetworkEntity(e.0), tick: Tick(1), position_mm: [9000, 0, 0], rotation_xyz_microunits: [1_000_001, 0, 0] };
        assert_eq!(r.apply_network_state(s), Err(NetworkApplyError::InvalidRotation));
    ...
    fn publish_rejects_non_finite_transform() {
        let mut r = runtime();
        let e = r.spawn(Transform { translation: [f32::NAN, 0.0, 0.0], ..Default::default() });
    ...
    fn publish_normalizes_rotation_before_quantization() {
        let mut r = runtime();
        let e = r.spawn(Transform { rotation_xyzw: [0.0, 0.0, 0.0, 2.0], ..Default::default() });
        r.publish_entity(e).unwrap();
        let packet = r.network.transport.drain().pop().unwrap();
        let (_, state) = Replicator::<atc_genesis_network::LoopbackTransport>::decode_packet(&packet).unwrap();
    ...
    fn unknown_network_entity_is_rejected() {
        let mut r = runtime();
        let e = EntityId(77);
        let s = ReplicatedState { entity: NetworkEntity(e.0), tick: Tick(1), position_mm: [0; 3], rotation_xyz_microunits: [0; 3] };
        assert_eq!(r.apply_network_state(s), Err(NetworkApplyError::UnknownEntity(e)));
    }
    ...
    fn receive_rejects_replay() {
        let mut r = runtime();
        let e = r.spawn(Transform::default());
        let s = ReplicatedState { entity: NetworkEntity(e.0), tick: Tick(1), position_mm: [1000, 0, 0], rotation_xyz_microunits: [0; 3] };
        r.network.publish(&s).unwrap();
        let p = r.network.transport.drain().pop().unwrap();
        assert!(r.receive_network_packet(&p).is_ok());
        assert_eq!(r.receive_network_packet(&p), Err(NetworkReceiveError::Rejected));
    }
    ...
    fn secure_receive_enforces_session_replay_and_entity() {
        let mut r = runtime();
        let e = r.spawn(Transform::default());
        let s = ReplicatedState { entity: NetworkEntity(e.0), tick: Tick(2), position_mm: [2000, 0, 0], rotation_xyz_microunits: [0; 3] };
        r.network.publish(&s).unwrap();
        let p = r.network.transport.drain().pop().unwrap();
        let mut guard = PacketGuard::new(PeerId(5), SessionId(9));
        assert_eq!(r.receive_secure_network_packet_for_entity(&mut guard, SessionId(8), NetworkEntity(e.0), &p), Err(SecureNetworkReceiveError::Rejected(PacketRejectReason::SessionMismatch)));
        assert_eq!(r.receive_secure_network_packet_for_entity(&mut guard, SessionId(9), NetworkEntity(99), &p), Err(SecureNetworkReceiveError::EntityMismatch { expected: NetworkEntity(99), received: NetworkEntity(e.0) }));
        assert!(r.receive_secure_network_packet_for_entity(&mut guard, SessionId(9), NetworkEntity(e.0), &p).is_ok());
        assert_eq!(r.receive_secure_network_packet_for_entity(&mut guard, SessionId(9), NetworkEntity(e.0), &p), Err(SecureNetworkReceiveError::Rejected(PacketRejectReason::Replay)));
    }
    ...
    fn gameplay_pipeline_remains_active() {
        ...
        r.queue_movement(e, MovementConfig { speed: 2.0, ..Default::default() });
    ...
    fn explicit_audio_constructor_still_works() {
        let r = GenesisRuntime::with_audio_and_network(NullRenderer::default(), NullAudioRuntime::default(), Replicator::new(atc_genesis_network::LoopbackTransport::default()), PhysicsConfig::default());
        assert_eq!(r.frame(), 0);
    }
}
