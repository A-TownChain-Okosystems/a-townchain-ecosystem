...
from core.ecs import MovementSystem, Position, Velocity, World
...
def test_add_and_query_components() -> None:
    world = World()
    eid = world.create_entity()
...
def test_movement_system_moves_entity() -> None:
    world = World()
...
def test_query_excludes_entities_missing_components() -> None:
    world = World()
    ...
    world.add_component(eid_partial, Position(5, 5))
    results = list(world.query(Position, Velocity))
    assert len(results) == 1
