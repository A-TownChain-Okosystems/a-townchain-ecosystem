            ...
            fn execute(&self, ctx: AgentContext) -> AgentResponse {
                AgentResponse::success(
                    ...
                    &format!(
                        "[{}] Processing: {}",
                        self.role(),
                        &ctx.input[..ctx.input.len().min(50)]
                    ),
                )
            }
        }
...
impl_agent!(
    ...
    &["plan", "prioritize", "spec"]
);
impl_agent!(
    "roadmap",
    RoadmapAgent,
    "Roadmap — sprint planning and milestones",
    &["plan_sprint", "track", "report"]
);
impl_agent!(
    "security",
    SecurityAgent,
...
impl_agent!(
    ...
    &["respond", "moderate", "engage"]
);
impl_agent!(
    "treasury",
    TreasuryAgent,
    "Treasury — fund management",
    &["allocate", "report", "audit"]
);
impl_agent!(
    "audit",
    AuditAgent,
...
mod tests {
    ...
    fn test_all_agents() {
        let agents: Vec<Box<dyn Agent>> = vec![
            Box::new(GovernanceAgent),
            Box::new(ProductAgent),
            Box::new(RoadmapAgent),
            Box::new(SecurityAgent),
            Box::new(AnalyticsAgent),
            Box::new(DevOpsAgent),
            Box::new(CommunityAgent),
            Box::new(TreasuryAgent),
            Box::new(AuditAgent),
            Box::new(ResearchAgent),
            Box::new(SupportAgent),
            Box::new(BridgeAgent),
        ];
