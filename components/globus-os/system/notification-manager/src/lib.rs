...
mod tests {
    ...
    #[test]
    fn bounded_fifo() {
        let mut m = NotificationManager::new(1);
        assert_eq!(m.push(1, "hello").unwrap(), NotificationId(1));
        assert_eq!(m.push(1, "x"), Err(NotificationError::QueueFull));
        assert_eq!(m.pop().unwrap().id, NotificationId(1));
    }
}
