...
fn wallet_sdk_and_blockchain_use_identical_l1_signing_bytes() {
    ...
    let sdk = TransactionBuilder::transfer(CHAIN_ID, "alice", "bob", 100, 1, 2_000, 0, 1)
        .poh_hash([7u8; 32])
        .sign(&sdk_key);
    ...
    assert!(verifier.verify(&wallet_bytes, &sdk.signature, &sdk.public_key));

    let node = Node::new(CHAIN_ID, "validator-1".into());
    node.state
        .genesis_credit("alice", 10_000)
