...
use crate::identity::{
    compute_genesis_id, verify_genesis_id, ChainIdentity, CHAIN_ID, DEVNET_NETWORK_ID,
...
impl Genesis {
    pub fn devnet() -> Self {
        ...
        let genesis_id = compute_genesis_id(
            CHAIN_ID,
            "A-TownChain Devnet",
            DEVNET_NETWORK_ID,
            0,
        ...
        Genesis {
            ...
            chain_name: "A-TownChain Devnet".into(),
    ...
    pub fn from_file(path: &str) -> Result<Self, String> {
        let content =
            std::fs::read_to_string(path).map_err(|e| format!("lesen {}: {}", path, e))?;
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.genesis_height != 0 {
            return Err(format!(
                "Genesis-Height muss 0 sein, ist {}",
                self.genesis_height
            ));
        }
        if self.chain_name.is_empty() {
            return Err("Chain-Name fehlt".into());
        }
        if self.initial_peers.len() < 2 {
            return Err("Devnet braucht mindestens 2 Initial-Peers".into());
        }
    ...
    fn canonical_boot_encoding(&self) -> Vec<u8> {
        let peers = serde_json::to_string(&self.initial_peers)
...
mod tests {
    ...
    fn genesis_file_bindung_ist_erzwungen() {
        let g = Genesis::from_file("config/devnet/genesis.json").unwrap();
        assert_eq!(g, Genesis::devnet());
        g.validate().unwrap();
    }
    #[test]
    ...
    fn zwei_nodes_gleiche_genesis_gleicher_hash() {
        let g = Genesis::devnet();
        let (a, h1) = devnet_boot(&g, &[(1, "a".into()), (2, "b".into())]).unwrap();
        let (b, h2) = devnet_boot(&g, &[(1, "a".into()), (2, "b".into())]).unwrap();
        assert_eq!(h1, h2);
        assert_eq!(a.len(), 2);
        assert_eq!(b.len(), 2);
    }
    ...
    fn duplikat_peer_abgelehnt() {
        let g = Genesis::devnet();
        assert!(devnet_boot(&g, &[(1, "a".into()), (1, "dup".into())]).is_err());
    }
}
