...
mod tests {
    ...
    #[test] fn genesis_id_is_deterministic() { let p=peers(); let a=compute_genesis_id(CHAIN_ID,"A-TownChain Devnet",DEVNET_NETWORK_ID,0,&p,"0".repeat(64).as_str(),PROTOCOL_VERSION,VM_VERSION); let b=compute_genesis_id(CHAIN_ID,"A-TownChain Devnet",DEVNET_NETWORK_ID,0,&p,"0".repeat(64).as_str(),PROTOCOL_VERSION,VM_VERSION); assert_eq!(a,b); assert_eq!(a.len(),64); }
    #[test] fn identity_is_fail_closed() { let p=peers(); let id=compute_genesis_id(CHAIN_ID,"A-TownChain Devnet",DEVNET_NETWORK_ID,0,&p,"0".repeat(64).as_str(),PROTOCOL_VERSION,VM_VERSION); let i=ChainIdentity{chain_id:CHAIN_ID.into(),network_id:DEVNET_NETWORK_ID.into(),genesis_id:id}; assert!(verify_genesis_id(&i,"A-TownChain Devnet",0,&p,"0".repeat(64).as_str(),PROTOCOL_VERSION,VM_VERSION).is_ok()); }
