...
class BootstrapClient:
        ...
        for bs_host, bs_port in self.bootstraps:
            try:
                ...
                if resp.get("type") == "PEER_LIST":
                    ...
                    logger.info(
                        f"[BootstrapClient] {len(self.peers)} Peers von {bs_host}:{bs_port}"
                    )
                    return self.peers
    ...
    def ping(self, bs_host: str, bs_port: int = 4001) -> bool:
        ...
        try:
            ...
            sock.settimeout(3.0)
            sock.sendto(
                json.dumps({"type": "PING", "node_id": self.node_id}).encode(),
                (bs_host, bs_port),
            )
    ...
    def get_peers(self, limit: int = 50) -> list[Peer]:
        ...
        for bs_host, bs_port in self.bootstraps:
            try:
                ...
                sock.settimeout(5.0)
                sock.sendto(
                    json.dumps({"type": "GET_PEERS", "limit": limit}).encode(),
                    (bs_host, bs_port),
                )
                data, _ = sock.recvfrom(65535)
                resp = json.loads(data.decode())
