...
#[derive(Clone,Copy,Debug,PartialEq,Eq)]pub enum TxStatus{Pending,Validated,InBlock,Rejected,Expired,InvalidSignature}
...
#[derive(Clone,Debug,PartialEq,Eq)]pub struct Account{pub balance:u64,pub staked:u64,pub nonce:u64}pub struct StateDb{accounts:Mutex<BTreeMap<String,Account>>,dao:Mutex<crate::dao_state::DaoState>,genesis_sealed:Mutex<bool>}impl StateDb{pub fn new()->Self{Self{accounts:Mutex::new(BTreeMap::new()),dao:Mutex::new(crate::dao_state::DaoState::new(1,5000).expect("valid default DAO config")),genesis_sealed:Mutex::new(false)}}pub fn genesis_credit(&self,id:&str,n:u64)->Result<(),String>{if *self.genesis_sealed.lock().unwrap(){return Err("genesis allocation is sealed".into())}let mut a=self.accounts.lock().unwrap();let current=a.values().try_fold(0u64,|acc,x|acc.checked_add(x.balance).and_then(|v|v.checked_add(x.staked))).ok_or("supply overflow".to_string())?;let new_supply=current.checked_add(n).ok_or("supply overflow".to_string())?;if new_supply>MAX_ATC_SUPPLY{return Err(format!("ATC supply cap exceeded: {new_supply} > {MAX_ATC_SUPPLY}"))}let x=a.entry(id.into()).or_insert(Account{balance:0,staked:0,nonce:0});x.balance=x.balance.checked_add(n).ok_or("balance overflow".to_string())?;Ok(())}pub fn seal_genesis(&self){*self.genesis_sealed.lock().unwrap()=true}pub fn total_supply(&self)->u64{self.accounts.lock().unwrap().values().fold(0u64,|acc,x|acc.saturating_add(x.balance).saturating_add(x.staked))}pub fn balance(&self,id:&str)->u64{self.accounts.lock().unwrap().get(id).map(|x|x.balance).unwrap_or(0)}pub fn nonce(&self,id:&str)->u64{self.accounts.lock().unwrap().get(id).map(|x|x.nonce).unwrap_or(0)}pub fn staked(&self,id:&str)->u64{self.accounts.lock().unwrap().get(id).map(|x|x.staked).unwrap_or(0)}pub fn root(&self)->[u8;32]{let a=self.accounts.lock().unwrap();let mut b=Vec::new();for(k,v)in a.iter(){b.extend_from_slice(&(k.len()as u32).to_be_bytes());b.extend_from_slice(k.as_bytes());b.extend_from_slice(&v.balance.to_be_bytes());b.extend_from_slice(&v.staked.to_be_bytes());b.extend_from_slice(&v.nonce.to_be_bytes())}let account_root=simple_hash(&b);let supply=self.total_supply();let dao_root=self.dao.lock().unwrap().root();let mut combined=Vec::from(b"ATC-STATE-V2");combined.extend_from_slice(&account_root);combined.extend_from_slice(&supply.to_be_bytes());combined.extend_from_slice(&dao_root);simple_hash(&combined)}pub fn apply_dao_payload(&self,payload:&[u8],block:u64,sender:&str)->Result<(),String>{
 ...
 let result=match effect{
  ...
  Some(crate::dao_state::DaoEffect::TreasuryDeposit{amount})=>{
   ...
   let x=a.entry(sender.to_owned()).or_insert(Account{balance:0,staked:0,nonce:0});
   if x.balance<amount{Err("insufficient balance for DAO treasury deposit".into())}else{x.balance-=amount;Ok(())}
  },
  Some(crate::dao_state::DaoEffect::TreasuryPayout{recipient,amount})=>{
   let mut a=self.accounts.lock().unwrap();
   let x=a.entry(recipient).or_insert(Account{balance:0,staked:0,nonce:0});
   x.balance=x.balance.checked_add(amount).ok_or("recipient balance overflow".to_string())
  }
 };
 if let Err(e)=result{self.restore(account_snapshot);let _=self.restore_dao(&dao_snapshot);return Err(e)}
 Ok(())
}pub fn dao_snapshot(&self)->Vec<u8>{self.dao.lock().unwrap().encode()}pub fn restore_dao(&self,bytes:&[u8])->Result<(),String>{*self.dao.lock().unwrap()=crate::dao_state::DaoState::decode(bytes)?;Ok(())}pub fn apply_batch(&self,txs:&[Transaction])->Result<(),MempoolError>{let mut a=self.accounts.lock().unwrap();let mut staged=a.clone();for tx in txs{Self::apply_to(&mut staged,tx)?;}*a=staged;Ok(())}pub fn snapshot(&self)->BTreeMap<String,Account>{self.accounts.lock().unwrap().clone()} pub fn restore(&self,snapshot:BTreeMap<String,Account>){*self.accounts.lock().unwrap()=snapshot} pub fn apply(&self,tx:&Transaction)->Result<(),MempoolError>{self.apply_batch(std::slice::from_ref(tx))}fn apply_to(a:&mut BTreeMap<String,Account>,tx:&Transaction)->Result<(),MempoolError>{let s=a.get(&tx.sender_did).cloned().unwrap_or(Account{balance:0,staked:0,nonce:0});if s.nonce!=tx.nonce{return Err(MempoolError::InvalidNonce{expected:s.nonce,got:tx.nonce})}let fee=tx.max_fee();match tx.tx_type{TxType::Transfer|TxType::Contract=>{let debit=match tx.tx_type{TxType::Contract=>fee,_=>tx.amount.saturating_add(fee)};if s.balance<debit.saturating_add(if tx.tx_type==TxType::Contract{tx.amount}else{0}){return Err(MempoolError::InsufficientBalance)}let mut ns=s;ns.balance-=debit;ns.nonce+=1;a.insert(tx.sender_did.clone(),ns);if tx.tx_type==TxType::Transfer{let r=tx.recipient_did.as_ref().ok_or(MempoolError::NoRecipient)?;let x=a.entry(r.clone()).or_insert(Account{balance:0,staked:0,nonce:0});x.balance=x.balance.checked_add(tx.amount).ok_or(MempoolError::InsufficientBalance)?}}TxType::Stake=>{if s.balance<tx.amount.saturating_add(fee){return Err(MempoolError::InsufficientBalance)}let mut ns=s;ns.balance-=tx.amount.saturating_add(fee);ns.staked=ns.staked.checked_add(tx.amount).ok_or(MempoolError::InsufficientStake)?;ns.nonce+=1;a.insert(tx.sender_did.clone(),ns)}TxType::Unstake=>{if s.staked<tx.amount||s.balance<fee{return Err(MempoolError::InsufficientStake)}let mut ns=s;ns.staked-=tx.amount;ns.balance=ns.balance.checked_add(tx.amount).and_then(|v|v.checked_sub(fee)).ok_or(MempoolError::InsufficientBalance)?;ns.nonce+=1;a.insert(tx.sender_did.clone(),ns)}}Ok(())}fn saturating_add(a:u64,b:u64)->u64{a.saturating_add(b)}}
