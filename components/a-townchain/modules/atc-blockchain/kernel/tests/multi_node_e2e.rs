...
impl PeerTransport for Link {
    fn broadcast(&self, message: NetworkMessage) -> Result<(), String> {
        self.messages.lock().map_err(|_| "link poisoned".to_string())?.push(message);
        Ok(())
    }
}
...
fn multi_node_block_and_state_commit_are_identical() {
    ...
    node_b.chain.genesis(genesis_a.clone()).expect("node B genesis");
    node_b.storage.commit(genesis_a.clone()).expect("node B persist genesis");
    node_b.state.seal_genesis();
    node_b.consensus.set_height(0);

    let link = Arc::new(Link::default());
    link.broadcast(NetworkMessage::Block(genesis_a.id)).expect("broadcast genesis");
    ...
    assert_eq!(node_a.chain.last().unwrap().id, node_b.chain.last().unwrap().id);
    assert_eq!(node_a.chain.last().unwrap().state_root, node_b.chain.last().unwrap().state_root);

    link.broadcast(NetworkMessage::Block(block.id)).expect("broadcast block");
    let messages = link.messages.lock().unwrap();
    assert!(messages.iter().any(|m| matches!(m, NetworkMessage::Block(id) if *id == block.id)));
}
