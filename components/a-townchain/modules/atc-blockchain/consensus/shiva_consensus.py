...
class ProofOfStake:
    ...
    def select_block_producer(self, slot: int) -> ValidatorInfo | None:
        """
    ...
    def cast_vote(self, validator_addr: str, block_hash: str, slot: int) -> ValidatorVote | None:
...
class ProofOfWork:
    ...
    def adjust_difficulty(self, block_height: int) -> int:
        ...
        """
        if (
            block_height % self.ADJUSTMENT_WINDOW != 0
            or len(self.block_times) < self.ADJUSTMENT_WINDOW
        ):
...
class ShivaConsensus:
    ...
    def produce_block(self, validator_addr: str, max_txs: int = 100) -> ATCBlock | None:
        """
        ...
        print("  [2/3] PoS — Validator-Auswahl...")
        _producer = self.pos.select_block_producer(self.slot)
        ...
        if not success:
            print("        ⚠️  PoW-Timeout nach 500k Iterationen")
            nonce = 0
        ...
        print(
            f"        Votes: {len(block.votes)} | Finalität: {ratio:.1%} | {'✅ FINAL' if is_final else '⏳ AUSSTEHEND'}"
        )
    ...
    def verify_block(self, block: ATCBlock) -> tuple[bool, str]:
        ...
        if len(self.chain) > 0:
            expected_prev = (
                self.chain[block.header.height - 1].block_hash
