...
fn sdk_node_mempool_consensus_vm_state_storage_indexer(){
 let chain_id=658467;let node=Node::new(chain_id,"validator-1".into());let indexer=Arc::new(MemoryIndexer::new());node.set_indexer(indexer.clone());node.state.genesis_credit("alice",1_000_000).expect("genesis allocation must respect supply cap");node.create_genesis(1).unwrap();
...
fn storage_restart_recovers_chain_and_state(){
 ...
 let node=Node::open_storage(658467,"validator-1".into(),&path).unwrap();node.state.genesis_credit("alice",1_000_000).expect("genesis allocation must respect supply cap");node.create_genesis(1).unwrap();
