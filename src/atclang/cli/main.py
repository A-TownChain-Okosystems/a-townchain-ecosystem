...
from atclang.ir import ir_hash, to_json_ir
...
def cmd_compile(args) -> int:
    ...
    if not SecurityGate().check(source, profile):
        print(
            f"SECURITY-GATE FAIL (Profil {profile}) — siehe 'atc check'",
            file=sys.stderr,
        )
        return 1
...
def cmd_check(args) -> int:
    ...
    for f in findings:
        print(
            f"[{f.severity.value.upper()}] {f.rule} Zeile {f.line}: {f.message}",
            file=sys.stderr,
        )
...
def main(argv=None) -> int:
    ...
    p_compile.add_argument(
        "--profile", default="consensus", choices=["consensus", "off_chain", "debug"]
    )
