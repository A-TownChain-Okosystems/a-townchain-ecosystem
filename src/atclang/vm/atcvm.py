...
from atclang.stdlib.primitives import ATCAddress, ATCPrimitives
...
def _build_stdlib_dispatch() -> dict[str, Callable]:
    ...
    return {
        ...
        # Crypto — Duplikate entfernt: gleiche Keys werden spaeter im Dict durch
        # ATCCrypto-Implementierungen (Stdlib ATC-94) definiert und haetten diese
        # Eintraege zur Laufzeit ohnehin still ueberschrieben (F601, ATC-STD-BUG-001).
        "ATC::Crypto::verify_jwt": lambda args: s.verify_jwt(str(args[0]) if args else ""),
        "ATC::Crypto::ECDSA::sign": lambda args: s.ecdsa_sign(
            args[0] if args else "", args[1] if len(args) > 1 else ""
        ),
        "ATC::Crypto::ECDSA::verify": lambda args: s.ecdsa_verify(
            args[0] if args else "",
            args[1] if len(args) > 1 else "",
            args[2] if len(args) > 2 else "",
        ),
        "ATC::Crypto::ECDSA::privkey_to_pubkey": lambda args: s.hash_sha256(
            args[0] if args else ""
        ),
        "ATC::Crypto::BIP39::to_mnemonic": lambda args: s.bip39_mnemonic(
            args[0] if args else b"", int(args[1]) if len(args) > 1 else 24
        ),
        # Rand
        ...
        # Net
        "ATC::Net::UDP::send": lambda args: s.net_send(
            str(args[0]) if args else "",
            int(args[1]) if len(args) > 1 else 0,
            args[2] if len(args) > 2 else b"",
        ),
        "ATC::Net::Kademlia::find_node": lambda args: s.kademlia_find_node(
            str(args[0]) if args else "",
        ...
        # Storage
        "ATC::Storage::store": lambda args: s.storage_store(
            str(args[0]) if args else "", args[1] if len(args) > 1 else None
        ),
        ...
        # RPC
        "ATC::RPC::call": lambda args: s.rpc_call(
            str(args[0]) if args else "", args[1] if len(args) > 1 else {}
        ),
        ...
        "ATC::Crypto::hmac_sha256": lambda args: (
            ATCCrypto.hmac_sha256(args[0], args[1]) if len(args) > 1 else ""
        ),
        "ATC::Crypto::base58_encode": lambda args: ATCCrypto.base58_encode(args[0]) if args else "",
        "ATC::Crypto::base58_decode": lambda args: (
            ATCCrypto.base58_decode(args[0]) if args else b""
        ),
        "ATC::Crypto::base64_encode": lambda args: ATCCrypto.base64_encode(args[0]) if args else "",
        "ATC::Crypto::base64_decode": lambda args: (
            ATCCrypto.base64_decode(args[0]) if args else b""
        ),
        ...
        "ATC::Crypto::verify": lambda args: (
            ATCCrypto.verify(args[0], args[1], args[2]) if len(args) > 2 else False
        ),
        "ATC::Crypto::random_bytes": lambda args: (
            ATCCrypto.random_bytes(int(args[0])) if args else ATCCrypto.random_bytes(32)
        ),
        "ATC::Crypto::random_int": lambda args: (
            ATCCrypto.random_int(int(args[0]), int(args[1])) if len(args) > 1 else 0
        ),
        "ATC::Crypto::address_from_pubkey": lambda args: (
            ATCCrypto.address_from_pubkey(args[0]) if args else ""
        ),
        "ATC::Crypto::is_valid_address": lambda args: (
            ATCCrypto.is_valid_address(args[0]) if args else False
        ),
        # ── Stdlib Module: Collections (ATC-94) ─────────
        "ATC::Collections::map_new": lambda args: ATCCollections.map_new(),
        "ATC::Collections::map_get": lambda args: (
            ATCCollections.map_get(args[0], args[1]) if len(args) > 1 else None
        ),
        "ATC::Collections::map_set": lambda args: (
            ATCCollections.map_set(args[0], args[1], args[2]) if len(args) > 2 else {}
        ),
        "ATC::Collections::map_contains": lambda args: (
            ATCCollections.map_contains(args[0], args[1]) if len(args) > 1 else False
        ),
        "ATC::Collections::map_keys": lambda args: ATCCollections.map_keys(args[0]) if args else [],
        "ATC::Collections::map_values": lambda args: (
            ATCCollections.map_values(args[0]) if args else []
        ),
        ...
        "ATC::Collections::array_push": lambda args: (
            ATCCollections.array_push(args[0], args[1]) if len(args) > 1 else []
        ),
        "ATC::Collections::array_pop": lambda args: (
            ATCCollections.array_pop(args[0]) if args else None
        ),
        "ATC::Collections::array_get": lambda args: (
            ATCCollections.array_get(args[0], int(args[1])) if len(args) > 1 else None
        ),
        "ATC::Collections::array_len": lambda args: (
            ATCCollections.array_len(args[0]) if args else 0
        ),
        "ATC::Collections::set_new": lambda args: ATCCollections.set_new(),
        "ATC::Collections::set_add": lambda args: (
            ATCCollections.set_add(args[0], args[1]) if len(args) > 1 else set()
        ),
        "ATC::Collections::set_contains": lambda args: (
            ATCCollections.set_contains(args[0], args[1]) if len(args) > 1 else False
        ),
        "ATC::Collections::queue_new": lambda args: ATCCollections.queue_new(),
        "ATC::Collections::queue_enqueue": lambda args: (
            ATCCollections.queue_enqueue(args[0], args[1]) if len(args) > 1 else []
        ),
        "ATC::Collections::stack_new": lambda args: ATCCollections.stack_new(),
        "ATC::Collections::stack_push": lambda args: (
            ATCCollections.stack_push(args[0], args[1]) if len(args) > 1 else []
        ),
        # ── Stdlib Module: Encoding (ATC-94) ───────────
        "ATC::Encoding::json_encode": lambda args: ATCEncoding.json_encode(args[0]) if args else "",
        "ATC::Encoding::json_decode": lambda args: (
            ATCEncoding.json_decode(args[0]) if args else None
        ),
        "ATC::Encoding::cbor_encode": lambda args: (
            ATCEncoding.cbor_encode(args[0]) if args else b""
        ),
        "ATC::Encoding::cbor_decode": lambda args: (
            ATCEncoding.cbor_decode(args[0]) if args else None
        ),
        ...
        "ATC::Math::mod_exp": lambda args: (
            ATCMath.mod_exp(args[0], args[1], args[2]) if len(args) > 2 else 0
        ),
        ...
        "ATC::Primitives::Address::new": lambda args: (
            ATCAddress(args[0]) if args else ATCAddress("")
        ),
        "ATC::Primitives::Address::is_valid": lambda args: (
            ATCPrimitives.is_valid_address(args[0]) if args else False
        ),
        "ATC::Primitives::Hash::compute": lambda args: (
            ATCPrimitives.compute_hash(args[0].encode() if isinstance(args[0], str) else args[0])
            if args
            else None
        ),
...
class ATCVM:
    ...
    def set_var(self, name: str, value: Any, frame: CallFrame | None):
        if frame:
    ...
    def _call_method(self, obj: Any, method: str, args: list[Any]) -> Any:
        ...
        if isinstance(obj, list):
            ...
            if method == "contains":
                return args[0] in obj
            if method == "get":
                return obj[args[0]] if args[0] < len(obj) else None
            if method == "to_string":
                return str(obj)
        ...
        if isinstance(obj, str):
            ...
            if method == "slice":
                return obj[args[0] : args[1]] if len(args) >= 2 else obj
            if method == "to_short_string":
                return obj[:20] + ("..." if len(obj) > 20 else "")
        ...
        if isinstance(obj, (int, float)):
            if method == "to_string":
                return str(obj)
            if method == "to_bytes":
                return struct.pack(">Q", int(obj) & 0xFFFFFFFFFFFFFFFF)
    ...
    def execute(self, instructions: list[Instruction], frame: CallFrame | None = None) -> Any:
        ...
        while ip < len(instructions):
            ...
            # ── Arithmetik ─────────────────────────────
                ...
                b, a = self.pop(), self.pop()
                self.push(int(a or 0) % int(b or 1))
            elif op == OP.POW:
                b, a = self.pop(), self.pop()
                self.push(int(a or 0) ** int(b or 0))
            elif op == OP.NEG:
            ...
            # ── Variablen ──────────────────────────────
                ...
                if frame and args[0] in frame.locals:
                    del frame.locals[args[0]]
                elif args[0] in self.globals:
                    del self.globals[args[0]]
            ...
            # ── Objekte ────────────────────────────────
                ...
                type_name = args[0]
                field_count = args[1] if len(args) > 1 else 0
                ...
                obj = self.pop()
                if isinstance(obj, dict):
                    self.push(obj.get(fname2))
                elif isinstance(obj, ATCObject):
                    self.push(obj.fields.get(fname2))
                else:
                    self.push(getattr(obj, fname2, None))
                ...
                if isinstance(obj, dict):
                    ...
                    obj.fields[fname2] = val
                else:
                    setattr(obj, fname2, val)
                self.push(obj)
            elif op == OP.HAS_KEY:
                key = self.pop()
                obj = self.pop()
                ...
                if target_type == "String":
                    self.push(str(val) if val is not None else "")
                elif target_type in ("Int", "UInt32", "UInt64", "UInt256"):
                    self.push(int(val) if val else 0)
                elif target_type == "Bool":
                    self.push(bool(val))
                elif target_type == "Bytes":
                    self.push(str(val).encode())
                else:
                    self.push(val)

            # ── String-Ops ─────────────────────────────
            elif op == OP.STR_LEN:
                self.push(len(str(self.pop() or "")))
            elif op == OP.STR_UPPER:
                self.push(str(self.pop() or "").upper())
            elif op == OP.STR_LOWER:
                self.push(str(self.pop() or "").lower())
                ...
                fmt = self.pop()
                try:
                    self.push(str(fmt or "").format(*fmtargs))
                except Exception:
                    self.push(str(fmt or ""))
            ...
            # ── ATC-spezifisch ─────────────────────────
                ...
                print(
                    f"  \033[38;5;51m📡 [{event_name}]\033[0m {' | '.join(str(a)[:60] for a in event_args)}"
                )
