...
from ..lexer.lexer import TT, ATCLexer, Token
...
class ATCParser:
    ...
    def error(self, msg: str):
        tok = self.current()
        raise SyntaxError(
            f"[ATCLang Parser] {msg} @ Zeile {tok.line}:{tok.col} (bekam: {tok.type.name} = {tok.value!r})"
        )
    ...
    def match(self, ttype: TT, value=None) -> Token | None:
        ...
        if self.match(TT.LT):
            ...
            if not self.match(TT.GT):
                ...
                if self.check(TT.RSHIFT):
                    ...
                    self.tokens.insert(
                        self.pos,
                        replace(self.tokens[self.pos - 1], type=TT.GT, value=">"),
                    )
                else:
                    self.expect(TT.GT)
            return node
    ...
    def parse_postfix(self) -> ASTNode:
        ...
        while True:
            if self.match(TT.LBRACKET):
                ...
                if self.check(TT.COLON) or self.check(TT.DOTDOT) or self.check(TT.DOTDOTEQ):
                    self.advance()
                    ...
                    idx = self.parse_expr()
                    if self.match(TT.COLON) or self.match(TT.DOTDOT) or self.match(TT.DOTDOTEQ):
                        if self.check(TT.RBRACKET):
                            self.advance()
                ...
                if (
                    self.current().type == TT.KEYWORD
                    or self.current().type == TT.IDENT
                    or self.current().type == TT.TYPE
                ):
    ...
    def parse_primary(self) -> ASTNode:
        ...
        if tok.type == TT.TYPE:
            ...
            while self.check(TT.DCOLON):
                ...
                if (
                    self.current().type == TT.KEYWORD
                    and self.current().value in ("new", "delete", "deploy", "call")
                    or self.check(TT.IDENT)
                    or self.check(TT.ATC_STD)
                    or self.check(TT.TYPE)
                ):
            ...
            if self.check(TT.LBRACE) and parts[0][0].isupper() and len(parts[0]) > 1:
                ...
                if (
                    self.current().type in (TT.IDENT, TT.TYPE, TT.KEYWORD)
    ...
    def parse_statement(self) -> ASTNode:
        ...
        if self.check(TT.KEYWORD, "break"):
            self.advance()
            return BreakStatement(tok.line, tok.col)
        if self.check(TT.KEYWORD, "continue"):
            self.advance()
            return ContinueStatement(tok.line, tok.col)
    ...
    def parse_if(self) -> IfStatement:
        ...
        if self.check(TT.KEYWORD, "else"):
            ...
            if self.check(TT.KEYWORD, "if"):
                ...
                while self.check(TT.KEYWORD, "elif") or (
                    self.check(TT.KEYWORD, "else")
    ...
    def parse_contract(self) -> ContractDef:
        ...
        while not self.check(TT.RBRACE) and not self.check(TT.EOF):
            if self.check(TT.KEYWORD, "state"):
                ...
                self.check(TT.KEYWORD, "pub")
                and self.peek().type == TT.KEYWORD
                and self.peek().value == "fn"
            ):
                self.advance()  # 'pub'
                functions.append(self.parse_function())
            else:
    ...
    def parse_match_expr(self):
        ...
        while not self.check(TT.RBRACE) and not self.check(TT.EOF):
            ...
            if self.check(TT.IDENT) and self.current().value == "_":
                ...
                pattern = str(self.advance().value)
            # Expect => or :
            if self.match(TT.FAT_ARROW) or self.match(TT.COLON):
                pass
