...
impl PacketGuard {
    pub fn new(peer: PeerId, session: SessionId) -> Self { Self { peer, session, sequence: PeerSequence { peer, last_sequence: 0, last_tick: Tick(0) } } }
    ...
    pub fn validate_for_entity(&mut self, session: SessionId, expected: NetworkEntity, bytes: &[u8]) -> Result<ValidatedPacket, PacketRejectReason> {
        self.validate_inner(session, Some(expected), bytes)
    }

    fn validate_inner(&mut self, session: SessionId, expected: Option<NetworkEntity>, bytes: &[u8]) -> Result<ValidatedPacket, PacketRejectReason> {
        ...
        if let Some(expected_entity) = expected { if state.entity != expected_entity { return Err(PacketRejectReason::EntityMismatch); } }
        if header.tick != state.tick || !state.has_valid_rotation() { return Err(PacketRejectReason::Malformed); }
...
mod tests {
    ...
    fn packet_for(entity: u64, tick: u64, sequence: u64) -> Vec<u8> {
        let header = ReplicationHeader { tick: Tick(tick), sequence, entity: NetworkEntity(entity) };
        let state = ReplicatedState { entity: NetworkEntity(entity), tick: Tick(tick), position_mm: [1, 2, 3], rotation_xyz_microunits: [4, 5, 6] };
        let mut bytes = Vec::new(); bytes.extend_from_slice(&header.encode()); bytes.extend_from_slice(&state.encode()); bytes
    }
    fn packet() -> Vec<u8> { packet_for(9, 4, 1) }
    ...
    #[test] fn rejects_stale_tick() { let mut g=PacketGuard::new(PeerId(7),SessionId(11)); assert!(g.validate(SessionId(11),&packet()).is_ok()); let b=packet_for(9,3,2); assert_eq!(g.validate(SessionId(11),&b),Err(PacketRejectReason::StaleTick)); }
    ...
    #[test] fn rejects_invalid_rotation() { let mut g=PacketGuard::new(PeerId(7),SessionId(11)); let mut b=packet(); let offset=ReplicationHeader::BYTES+28; b[offset..offset+4].copy_from_slice(&1_000_001i32.to_le_bytes()); assert_eq!(g.validate(SessionId(11),&b),Err(PacketRejectReason::Malformed)); assert_eq!(g.last_sequence(),0); }
    #[test] fn unauthorized_entity_does_not_advance_sequence() { let mut g=PacketGuard::new(PeerId(7),SessionId(11)); let wrong=packet_for(99,4,1); assert_eq!(g.validate_for_entity(SessionId(11),NetworkEntity(9),&wrong),Err(PacketRejectReason::EntityMismatch)); assert_eq!(g.last_sequence(),0); assert_eq!(g.last_tick(),Tick(0)); let allowed=packet_for(9,4,1); assert!(g.validate_for_entity(SessionId(11),NetworkEntity(9),&allowed).is_ok()); assert_eq!(g.last_sequence(),1); }
}
