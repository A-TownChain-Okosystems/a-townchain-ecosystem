...
impl ReplicationHeader{pub fn encode(self)->[u8;24]{let mut b=[0;24];b[0..8].copy_from_slice(&self.tick.0.to_le_bytes());b[8..16].copy_from_slice(&self.sequence.to_le_bytes());b[16..24].copy_from_slice(&self.entity.0.to_le_bytes());b}pub fn decode(b:&[u8])->Option<Self>{if b.len()<24{return None}Some(Self{tick:Tick(u64::from_le_bytes(b[0..8].try_into().ok()?)),sequence:u64::from_le_bytes(b[8..16].try_into().ok()?),entity:NetworkEntity(u64::from_le_bytes(b[16..24].try_into().ok()?))})}}
...
#[derive(Default)]pub struct Replicator<T:ReplicationTransport>{pub transport:T,sequence:u64}
impl<T:ReplicationTransport>Replicator<T>{pub fn new(transport:T)->Self{Self{transport,sequence:0}}pub fn publish(&mut self,state:&ReplicatedState)->Result<u64,String>{self.sequence=self.sequence.saturating_add(1);let h=ReplicationHeader{tick:state.tick,sequence:self.sequence,entity:state.entity};let mut p=Vec::with_capacity(64);p.extend_from_slice(&h.encode());p.extend_from_slice(&state.encode());self.transport.send(&p)?;Ok(self.sequence)}}
#[cfg(test)]mod tests{use super::*;#[test]fn buffer(){let mut b=SnapshotBuffer::with_capacity(2);b.push(Snapshot{tick:Tick(0),sequence:1,value:1});b.push(Snapshot{tick:Tick(1),sequence:2,value:2});b.push(Snapshot{tick:Tick(2),sequence:1,value:3});assert_eq!(b.latest().unwrap().value,2)}#[test]fn header(){let h=ReplicationHeader{tick:Tick(4),sequence:7,entity:NetworkEntity(9)};assert_eq!(ReplicationHeader::decode(&h.encode()),Some(h))}#[test]fn state(){let s=ReplicatedState{entity:NetworkEntity(2),tick:Tick(8),position_mm:[1,-2,3],rotation_millirad:[4,5,-6]};assert_eq!(ReplicatedState::decode(&s.encode()),Some(s))}#[test]fn interpolation(){let a=ReplicatedState{entity:NetworkEntity(1),tick:Tick(1),position_mm:[0;3],rotation_millirad:[0;3]};let b=ReplicatedState{entity:NetworkEntity(1),tick:Tick(2),position_mm:[1000,2000,3000],rotation_millirad:[100,200,300]};let p=InterpolatedTransform::between(&a,&b,0.5).unwrap();assert_eq!(p.position,[0.5,1.0,1.5])}#[test]fn prediction(){let mut p=PredictionBuffer::with_capacity(4);p.push(Tick(1),[1.0,0.0,0.0]);p.push(Tick(2),[2.0,0.0,0.0]);assert_eq!(p.inputs_after(Tick(1)).count(),1)}}
