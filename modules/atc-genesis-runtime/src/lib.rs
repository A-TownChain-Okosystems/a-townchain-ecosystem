...
use atc_genesis_network::security::{validate_entity_owner, PacketGuard, PacketRejectReason, SessionId};
...
impl<R, A, N> GenesisRuntime<R, A, N> {
    ...
    pub fn receive_secure_network_packet_for_entity(&mut self, guard: &mut PacketGuard, session: SessionId, expected: NetworkEntity, bytes: &[u8]) -> Result<(), SecureNetworkReceiveError> {
        let validated = guard.validate(session, bytes).map_err(SecureNetworkReceiveError::Rejected)?;
        if !validate_entity_owner(expected, &validated.state) {
            return Err(SecureNetworkReceiveError::EntityMismatch { expected, received: validated.state.entity });
        }
        self.apply_network_state(validated.state).map_err(SecureNetworkReceiveError::Apply)
    }
}
