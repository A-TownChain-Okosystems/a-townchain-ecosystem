...
impl GameplayRuntime {
    pub fn sample_movement(&mut self, input: &InputState, entity: EntityId, config: MovementConfig) -> GameplayCommand {
        ...
        let command = GameplayCommand { entity, movement: [x * config.speed, 0.0, z * config.speed] };
...
mod tests {
    ...
    fn keyboard_moves_entity_deterministically() {
        ...
        g.sample_movement(&input,e,MovementConfig{speed:2.0,..Default::default()});
    ...
    fn axis_is_clamped() {
        ...
        let e=EntityId(1);
        let mut g=GameplayRuntime::default();
        let c=g.sample_movement(&input,e,MovementConfig{speed:2.0,axis_x:Some(1),..Default::default()});
    ...
    fn diagonal_movement_preserves_max_speed() {
        ...
        let mut g=GameplayRuntime::default();
        let c=g.sample_movement(&input,EntityId(1),MovementConfig{speed:6.0,..Default::default()});
