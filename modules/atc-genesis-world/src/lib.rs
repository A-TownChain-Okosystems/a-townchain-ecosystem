...
impl WorldStreamer {
    ...
    pub fn loading(&self) -> impl Iterator<Item = &WorldChunk> {
        self.chunks.iter().filter(|c| c.state == ChunkState::Loading)
    }
    ...
    pub fn plan_transitions<R: ChunkAssetResolver>(
        &mut self,
        position: [f32; 3],
        load_distance: f32,
        unload_distance: f32,
        ...
        for chunk in &mut self.chunks {
            if chunk.state == ChunkState::Loaded
                && chunk.bounds.distance_squared(position) > unload2
            {
                chunk.state = ChunkState::Unloaded;
                transitions.push(ChunkTransition::Unloaded(chunk.id));
            }
        }
        ...
        let mut candidates: Vec<(f32, WorldChunkId)> = self
            .chunks
            .iter()
            .filter(|c| c.state == ChunkState::Unloaded && assets.contains(c.asset))
            .map(|c| (c.bounds.distance_squared(position), c.id))
            .filter(|(distance, _)| *distance <= load2)
            .collect();
        candidates.sort_by(|a, b| a.0.total_cmp(&b.0).then_with(|| a.1.cmp(&b.1)));
...
mod tests {
    ...
    impl ChunkAssetResolver for Assets {
        fn contains(&self, asset: AssetId) -> bool {
