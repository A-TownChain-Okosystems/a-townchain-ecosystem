...
#[derive(Default)]
pub struct World { next: u64, transforms: HashMap<EntityId, Transform>, parents: HashMap<EntityId, Parent> }
impl World {
    ...
    pub fn set_parent(&mut self,child:EntityId,parent:Option<EntityId>)->bool{if !self.transforms.contains_key(&child)||parent.is_some_and(|p|!self.transforms.contains_key(&p)){return false;}match parent{Some(p)=>{self.parents.insert(child,Parent(p));},None=>{self.parents.remove(&child);}}true}
    ...
    pub fn world_transform(&self,id:EntityId)->Option<Transform>{let mut current=id;let mut result=*self.transforms.get(&id)?;let mut guard=0;while let Some(parent)=self.parents.get(&current){guard+=1;if guard>self.transforms.len(){return None;}let parent_transform=*self.transforms.get(&parent.0)?;result=combine(parent_transform,result);current=parent.0;}Some(result)}
}
...
impl<R:atc_genesis_platform::Renderer> TransformRenderPipeline<R>{pub fn render(&mut self,frame:atc_genesis_platform::FrameId){self.renderer.begin_frame(frame);let mut ids=self.world.iter_transforms().map(|(id,_)|id).collect::<Vec<_>>();ids.sort_by_key(|id|id.0);for id in ids{if let Some(world_transform)=self.world.world_transform(id){self.renderer.submit(id,world_transform);}}self.renderer.end_frame();}}
#[cfg(test)]mod tests{use super::*;#[test]fn hierarchy_resolves(){let mut w=World::new();let p=w.spawn(Transform{translation:[2.0,0.0,0.0],..Default::default()});let c=w.spawn(Transform{translation:[1.0,0.0,0.0],..Default::default()});assert!(w.set_parent(c,Some(p)));assert_eq!(w.world_transform(c).unwrap().translation[0],3.0);}#[test]fn imported_id_is_preserved(){let mut w=World::new();assert!(w.insert(EntityId(42),Transform::default()));assert_eq!(w.transform(EntityId(42)),Some(&Transform::default()));}}
