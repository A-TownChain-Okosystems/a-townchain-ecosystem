...
impl AssetPipeline {
    ...
    /// Validate the complete transitive dependency graph of an asset.
    ///
    /// Validation fails for missing dependencies, self-dependencies and cycles.
    /// Traversal order is sorted by AssetId so the result is deterministic.
    ...
    /// Return a deterministic topological cooking order for an asset graph.
...
impl MeshAsset {
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.indices.iter().any(|i| *i as usize >= self.vertices.len()) {
            return Err("index out of range".into());
        }
        if self.vertices.iter().chain(self.normals.iter()).any(|v| v.iter().any(|x| !x.is_finite())) {
