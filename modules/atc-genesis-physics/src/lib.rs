...
impl PhysicsSimulation{
 pub fn new(config:PhysicsConfig)->Self{Self{config:PhysicsConfig{fixed_dt:config.fixed_dt.max(1e-6),..config},..Self::default()}}
 ...
 pub fn step_fixed(&mut self){let dt=self.config.fixed_dt;for body in &mut self.bodies{if body.dynamic{for i in 0..3{body.velocity[i]+=self.config.gravity[i]*dt;body.position[i]+=body.velocity[i]*dt;}}}}
 pub fn advance(&mut self,dt_seconds:f32)->u32{self.accumulator+=dt_seconds.clamp(0.0,1.0);let mut steps=0;while self.accumulator>=self.config.fixed_dt&&steps<self.config.max_substeps{self.step_fixed();self.accumulator-=self.config.fixed_dt;steps+=1;}steps}
