...
impl RigidBody {
    pub fn dynamic(entity: EntityId, position: [f32; 3], mass: f32) -> Self {
        Self { entity, position, velocity: [0.0; 3], mass: mass.max(0.0001), dynamic: true }
    }
}
...
impl Default for PhysicsConfig {
    fn default() -> Self { Self { fixed_dt: 1.0 / 60.0, gravity: [0.0, -9.81, 0.0], max_substeps: 8 } }
}
...
impl PhysicsSimulation {
    ...
    pub fn step_fixed(&mut self) {
        ...
        for body in &mut self.bodies {
            if body.dynamic {
                body.velocity[0] += self.config.gravity[0] * dt;
                body.velocity[1] += self.config.gravity[1] * dt;
                body.velocity[2] += self.config.gravity[2] * dt;
                for i in 0..3 { body.position[i] += body.velocity[i] * dt; }
            }
        }
    }
    pub fn advance(&mut self, dt_seconds: f32) -> u32 {
        self.accumulator += dt_seconds.max(0.0).min(1.0);
        let mut steps = 0;
...
impl PhysicsWorld for PhysicsSimulation {
    fn step(&mut self, dt_seconds: f32) { let _ = self.advance(dt_seconds); }
